...
# %% auto 0
__all__ = ['DAISY_ROOT', 'AlertT', 'Alert', 'StepsT', 'StepT', 'Steps', 'LiStep', 'LoadingT', 'Loading', 'ToastHT', 'ToastVT',
           'Toast']
...
def Alert(*c, # Content for Alert (often text and/or icon)
    ...
    return Div(Span(*c), daisy_data_root="true", cls=('alert', stringify(cls)), role='alert', **kwargs)
...
def Steps(*li, # Each `Li` represent a step (generally use `LiStep`)
    ...
    return Ul(*li, daisy_data_root="true", cls=('steps',stringify(cls)), **kwargs)
...
def Loading(cls=(LoadingT.bars, LoadingT.lg), # Classes for indicator (generally `LoadingT` options)
    ...
    return Span(daisy_data_root=True, cls=classes, **kwargs)
...
def Toast(*c, # Content for toast (often test)
    ...
    return Div(a, NotStr(f"<script>{js}</script>"), daisy_data_root=True, data_mui=_id, cls=('toast', stringify(cls)), **kwargs)
